...
Data <- inner_join(death_source, fuel_gdp, by = c('Entity', "Year")) %>% 
  group_by(Entity) %>%
...
Risk_access <- Data %>% 
  ggplot(aes(y = Risk, x = Access)) + geom_smooth(alpha = 0.5, color = '#000084', fill = 'lightgray') +
  ...
  annotate(geom = 'text', 
           label = paste('R =',cor_Access_risk), x = 30, y = 50, size = 12,
           color = '#000084', family = 'anton') +
...
Access_GDP <- Data %>% 
  ...
  theme(plot.background  = element_blank(),
        ...
        axis.text.y      =  element_text(family = 'anton', 
                                         colour = "gray"))
Risk_GDP <- Data %>% 
  ggplot(aes(y = Risk, x = GDP)) + geom_smooth(alpha = 0.5, color = '#757A62', fill = 'lightgray') +
  scale_x_log10()  +
  ...
  xlab('GDP per capita') +
  annotate(geom = 'text', label = paste('R =',cor_GDP_risk), 
           x = 100, y = 100, size = 7, color = '#757A62', family = 'anton') +
  ylab('Mortality risk by\nindoor air pollution') +
  theme(plot.background  = element_blank(),
        panel.background = element_blank(),
        legend.position  = "none",
...
extra <- ggdraw() + 
  draw_label("...this is not due to an increment of GDP...",hjust = 0.8, vjust = -1.4,
            size = 15,
            fontfamily = 'anton')
